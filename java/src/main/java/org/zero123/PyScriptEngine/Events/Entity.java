...
//                args.addProperty("itemName", BuiltInRegistries.ITEM.getKey(itemEntity.getItem().getItem()).toString());
        ...
        {
            EntityUtil.addClientTempEntity(uuid, entity);
            EventManager.callClientJsonEvent(3, args.toString());
            EntityUtil.removeClientTempEntity(uuid);
        }
        else
        {
            EntityUtil.addServerTempEntity(uuid, entity);
            EventManager.callServerJsonEvent(3, args.toString());
            EntityUtil.removeServerTempEntity(uuid);
        }
    }
    ...
    public void onLivingIncomingDamage(LivingIncomingDamageEvent event)
    {
        ...
        // 计算伤害源的实体
        args.addProperty("srcId", EntityUtil.getEntityStrId(source.getEntity()));
        // 计算间接实体(通常是投掷物)
        args.addProperty("projectileId", EntityUtil.getEntityStrId(source.getDirectEntity()));
        // 受伤的实体
        args.addProperty("entityId", EntityUtil.getEntityStrId(event.getEntity()));
        // damage值(允许修改)
