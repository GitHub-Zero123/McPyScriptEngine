...
class QEntityCompService(BaseService):
    ...
    def getMemoryLiveState(self, entityId):
        ...
        return serverApi.GetEngineCompFactory().CreateGame(levelId).IsEntityAlive(entityId)
...
class QBaseEntityComp(_QBaseEntityComp):
    ...
    def getNeedUpdate(self):
        # 在NeoForge环境下Tick古代20/s不再需要区分判断
    ...
    def bind(self, entityId=""):
        if self.entityId:
            return False
        # NeoForge下 实体新增事件必须在下一tick才会完成level uuid注册，因此此处直接忽略前置判断交给update的判断处理
        # comp = serverApi.GetEngineCompFactory().CreateGame(levelId)
        # alive = comp.IsEntityAlive(entityId)
        # if not alive or not self._preVerification(entityId):
        #     return False
        if not self._preVerification(entityId):
            return False
        self.entityId = entityId
